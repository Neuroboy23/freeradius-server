/*
 * rlm_eap_eagent.c    Handles that are called from eap
 *
 * Version:     $Id$
 *
...
/*
 *	EAP-EAGENT is just ASCII data carried inside of the EAP session.
 *	The length of the data is indicated by the encapsulating EAP
 *	protocol.
 */
typedef struct rlm_eap_eagent_t {
...
static CONF_PARSER module_config[] = {
	{ "challenge", FR_CONF_OFFSET(PW_TYPE_STRING, rlm_eap_eagent_t, challenge), "Password: " },
	{ "auth_type", FR_CONF_OFFSET(PW_TYPE_STRING, rlm_eap_eagent_t, auth_type_name), "PAP" },
...
{
	rlm_eap_eagent_t	*inst;
	DICT_VALUE	*dval;

	*instance = inst = talloc_zero(cs, rlm_eap_eagent_t);
	if (!inst) return -1;
	...
	if (inst->auth_type_name && *inst->auth_type_name) {
		...
		if (!dval) {
			ERROR("rlm_eap_eagent: Unknown Auth-Type %s",
			      inst->auth_type_name);
			return -1;
		}
...
/*
 *	Initiate the EAP-EAGENT session by sending a challenge to the peer.
 */
static int mod_session_init(void *instance, eap_handler_t *handler)
{
	...
	rlm_eap_eagent_t *inst = (rlm_eap_eagent_t *) instance;
...
static int CC_HINT(nonnull) mod_process(void *instance, eap_handler_t *handler)
{
	...
	if (eap_ds->response->length <= 4) {
		ERROR("rlm_eap_eagent: corrupted data");
	...
	if (inst->auth_type == PW_AUTH_TYPE_LOCAL) {
		 ...
		 *	why the heck are they using EAP-EAGENT?
		 */
		...
		ERROR("rlm_eap_eagent: Response is too large to understand");
		eap_ds->request->code = PW_EAP_FAILURE;
		return 0;
 ...
 */
extern rlm_eap_module_t rlm_eap_eagent;
rlm_eap_module_t rlm_eap_eagent = {
	.name		= "eap_eagent",
